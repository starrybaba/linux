...
int main()
 {
         struct sockaddr_in serv_addr, cli_addr;//处理网络通信地址
 //      int len = sizeof(cli_addr), i;
         int i;
         time_t ticks;
         pthread_t thread;//线程id定义
         char buff[BUFFSIZE];
         ...
         serv_addr.sin_addr.s_addr = htonl(INADDR_ANY);//转换数据格式
